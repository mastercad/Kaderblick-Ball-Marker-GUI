"""Progress dialog for exporting heatmap training data."""
...
class _HeatmapExportWorker(QObject):
    ...
    def run(self):
        try:
            from training.export_heatmap_data import export_heatmap_dataset
...
class HeatmapExportDialog(QDialog):
    """Shows progress while heatmap samples are exported in a background thread."""

    def __init__(self, parent=None, json_path: str = "", output_dir: str = ""):
        super().__init__(parent)
        self.setWindowTitle("Heatmap-Trainingsdaten exportieren")
        self.setMinimumWidth(540)
        self._thread: QThread | None = None
        self._worker: _HeatmapExportWorker | None = None
        self._running = False
        ...
        self._status_label = QLabel("Heatmap-Export wird vorbereitet...")
    ...
    def _start(self, json_path: str, output_dir: str):
        self._running = True
        self._append("Export gestartet: Sequenzen, Heatmaps und Negativbeispiele.")
        self._thread = QThread(self)
        self._worker = _HeatmapExportWorker(json_path, output_dir)
    ...
    def _on_finished(self, stats: dict):
        ...
        self._status_label.setText("Heatmap-Export abgeschlossen.")
        self._append(
    ...
    def _on_failed(self, message: str):
        ...
        self._status_label.setText("Heatmap-Export fehlgeschlagen.")
        self._append(f"Fehler: {message}")
        self._cancel_btn.setEnabled(False)
        self._close_btn.setEnabled(True)
        QMessageBox.critical(self, "Heatmap-Export fehlgeschlagen", message)

    def _on_cancelled(self):
        self.was_cancelled = True
        self._running = False
        self._status_label.setText("Heatmap-Export abgebrochen.")
    ...
    def _cancel_or_close(self):
        if not self._running:
            self.reject()
            return
        self._status_label.setText("Heatmap-Export wird abgebrochen...")
